...
describe("upDevGroupNFT", () => {
  ...
  it("Should deploy", async () => {
    const factory = await ethers.getContractFactory("upDevGroupNFT");
  ...
  it("Should mint", async () => {
    ...
    const [whitelist, metadata, parent] = await nft.getDataBatchForTokenIds(new Array(3).fill(tokenId), [
      _LSP4_WHITELIST_KEY,
      _LSP4_METADATA_KEY,
      _LSP4_PARENT_KEY,
    ]);
  ...
  it("Should only mint with parentTokenId if user is admin", async () => {
    await expect(nft.connect(signers[1]).mint("0x01", "0x02", tokenId)).to.be.revertedWithCustomError(
      nft,
      "NotAllowed",
    );
  });
  ...
  it("Should set", async () => {
    ...
    const [whitelist, metadata, archive] = await nft.getDataBatchForTokenIds(new Array(3).fill(tokenId), [
      _LSP4_WHITELIST_KEY,
      _LSP4_METADATA_KEY,
      _LSP4_ARCHIVE_KEY,
    ]);
